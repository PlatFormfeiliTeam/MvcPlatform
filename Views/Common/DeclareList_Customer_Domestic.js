...
function gridpanelBind() {
    var store = Ext.create('Ext.data.JsonStore', {
        ...
        proxy: {
            type: 'ajax',
            url: '/Common/LoadDeclarationList_C_Domestic',
            reader: {
                root: 'rows',
                type: 'json',
                totalProperty: 'total'
...
function FileConsult(ORDERCODE, BUSITYPE, PREDECLCODE) {
    opencenterwin("/Common/FileConsult_C?menuxml=dec_domestic&source=declare&ORDERCODE=" + ORDERCODE + "&BUSITYPE=" + BUSITYPE + "&PREDECLCODE=" + PREDECLCODE, 1200, 900);
}
...
function ExportDecl() {
    ...
    Ext.Ajax.request({
        url: '/Common/ExportDeclList_C_Domestic',
        params: data,
        ...
        Ext.MessageBox.confirm("提示", "全部导出时可能因为文件过多而下载缓慢，确定导出吗？", function (btn) {
            if (btn == 'yes') {
                    ...
                    formtemp.submit({
                        ...
                        waitTitle: '请稍后...',
                        waitMsg: '正在下载,请稍后...',
                        url: '/Common/ExpDeclarationList_C_Domestic_all',
                        method: 'post',
                        params: {
                            CONDITION1: Ext.getCmp('CONDITION1').getValue(), VALUE1: Ext.getCmp("CONDITION1_1").getValue(),
                            CONDITION2: Ext.getCmp('CONDITION2').getValue(), VALUE2: Ext.getCmp("CONDITION2_1").getValue(),
...
function Searchstatus() {
    ...
    openrwindow("/Common/ClearanceStatus?menuxml=cus_dec_cle_domestic&id=" + id + "&rowIndex=" + index + "&currentPage=" + currentPage, 1200, 800);
}
